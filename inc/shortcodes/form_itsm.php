<?php
  ...
  add_shortcode( 'form_itsm', 'form_itsm_shortcode' );


  function form_itsm_shortcode($target_path) {
    ob_start();

    $urlToken = 'https://panelas-app2:8443/api/jwt/login';

    $responseToken = wp_remote_post( $urlToken, [
      'method'      => 'POST',
      'sslverify'   => FALSE,
      'timeout'     => 100,
      'redirection' => 5,
    ...
    if ( is_wp_error( $responseToken ) ) {
      echo $responseToken->get_error_message();
      return false;
    } else {
      $responceData = wp_remote_retrieve_body( $responseToken );
    }

	  $finalize_form = home_url( 'formulario-concluido/');
    $subjects = [
		'Cadastro/HOD',
		'Cadterc',
		'Catálogo',
...
<section id="itsm-form">
  <form class="row" id="form_itsm_old" method="post" action="<?php echo $finalize_form; ?>" enctype="multipart/form-data">
    ...
    <div class="col-md-12 mb-3 item-validation">
     ...
     <div class="col-md-12 mb-3 item-validation campo_anexo">
<!-- 		<label for="file" class="form-label">Anexar Evidência (Tipos: doc, docx, pdf, jpg, png | Tamanho: 2MB)</label> -->
    ...
    <div class="col-sm-12 col-md-3">
      <button id="enviaform" type="submit" class="btn btn-primary mb-2">Enviar</button>
    </div>
  </form>
...
<script>
	...
	const arqMaior2Megas = document.createElement('span');
	const anexoCampo = document.querySelector('.campo_anexo');
	fileInput.addEventListener('change', function() {
		if (this.files[0].size > 2 * 1024 * 1024) {
			alertaModal();
			anexoCampo.appendChild(arqMaior2Megas);
			this.value = null; 
		}else{
