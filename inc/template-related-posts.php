 ...
 * @return array
 */
function Prodesp_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = Prodesp_Theme()->customizer->get_value( 'related_posts_visible' );
	...
	$post_terms  = array();
	$post_number = Prodesp_Theme()->customizer->get_value( 'related_posts_count' );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = Prodesp_Theme()->customizer->get_value( $setting_value );
	}
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$image = ( $settings['image_visible'] ) ? Prodesp_post_thumbnail( 'Prodesp-thumb-s', array( 'echo' => false ) ) : '';
			...
			$author = ( $settings['author_visible'] ) ? Prodesp_posted_by( array( 'echo' => false ) ) : '';

			$date = ( $settings['date_visible'] ) ? Prodesp_posted_on( array( 'echo' => false ) ) : '';
